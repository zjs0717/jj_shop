...
async function load(): Promise<void> {
  ...
  error.value = ''
  try {
    const data = await fetchMyVideosApi('published')
...
async function unpublish(item: UserVideoItem): Promise<void> {
  actingId.value = item.id
  try {
    await updateMyVideoApi(item.id, { status: 'library' })
    await load()
  } catch (e) {
...
async function remove(item: UserVideoItem): Promise<void> {
  if (!confirm(`删除已发布作品「${item.title}」？`)) return
  actingId.value = item.id
  try {
  ...
  <div class="page">
    ...
    <header class="head">
      ...
      <div class="head__actions">
        ...
        <button type="button" class="link" @click="router.push('/home/me/library')">视频库</button>
      </div>
    </header>
    ...
    <div v-else-if="!list.length" class="empty">
      <p>还没有发布作品</p>
      <button type="button" @click="router.push('/home/me/upload')">去上传发布</button>
    </div>
    ...
    <div v-else class="grid">
      <article v-for="item in list" :key="item.id" class="card">
        <div class="card__media">
          ...
          <div class="stats">
            <span>{{ formatCount(item.playCount) }} 播放</span>
