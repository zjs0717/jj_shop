...
const isFormValid = (data: LoginForm): boolean => {
  return Boolean(data.username.trim() && data.password)
}
...
const handleSubmit = async (): Promise<void> => {
  ...
  errorMessage.value = ''
  try {
    await loginApi(buildPayload(form))
      ...
      <div class="auth__panel-inner">
        ...
        <form class="auth__form" @submit.prevent="handleSubmit">
          ...
          <div class="auth__options">
            ...
            <a href="#" class="auth__link" @click.prevent>忘记密码？</a>
          </div>
        ...
        <footer class="auth__foot">
          <span>还没有账户？</span>
          <a href="#" class="auth__link" @click.prevent="goRegister">立即注册</a>
        </footer>
      </div>
    </section>
  </div>
...
.auth__panel {
  ...
  padding: 40px 28px;
  background:
    radial-gradient(ellipse 80% 50% at 100% 0%, rgba(255, 45, 122, 0.16), transparent 55%),
